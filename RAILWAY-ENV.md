...
- The writing engine in `src/lib/engine.mjs` honors `OPENAI_API_KEY`, `OPENAI_BASE_URL`, `OPENAI_MODEL`. It rejects any `OPENAI_BASE_URL` pointing at `manus.im`, `api.anthropic.com`, or `fal.ai` — DeepSeek only.
- `FAL_KEY` is present for the image-tooling pipeline (Bunny image-prep scripts). The engine never touches it.
- `BUNNY_PULL_ZONE` is the read-only public CDN host. Article bodies live at `BUNNY_PULL_ZONE/immune-rebuilt/articles/<slug>.json`; images at `BUNNY_PULL_ZONE/immune-rebuilt/<slug>.webp`.
